...
![k3.png](..//public/k3.png)

::: info BLE Device — Read-only
The K3 communicates via Bluetooth Low Energy (BLE). It is **read-only** — Localkit receives state from the device but cannot send commands to it directly. The K3 can only be linked to a Pura Max. It is triggered by the linked Pura Max after each cleaning cycle, or manually via the Pura Max buttons.
:::

The Petkit K3 is a Bluetooth odor spray device designed to work alongside the [Pura Max](../devices/pura-max) litter box. It releases a deodorizing mist after each cleaning cycle. Localkit exposes its consumable levels and battery as Home Assistant sensors.
...
To pair or unpair, use the **Link with K3** / **Unlink from K3** actions on the [Pura Max](../devices/pura-max#k3-integration) device page in Localkit.
